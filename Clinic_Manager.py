...
onedrive_folder = os.path.expanduser(r'C:\Users\Aniruddh\OneDrive\Shree Brahmi')
...
def quit_app():
    ...
    try:
        ...
        file_name, file_ext = os.path.splitext(os.path.basename(db_file))
        destination_file = os.path.join(onedrive_folder, os.path.basename(db_file))
        old_file = os.path.join(onedrive_folder, file_name + "_old" + file_ext)
        # Remove the old version if it exists
        if os.path.exists(old_file):
            os.remove(old_file)
        # Rename the current version to "old" if it exists
        if os.path.exists(destination_file):
            os.rename(destination_file, old_file)
        shutil.copy2(db_file, destination_file)
        print("Database file copied to OneDrive successfully.")
    except Exception as e:
        print(f"Error copying database file to OneDrive: {e}")
...
add_button = Button(frame, text="Add Transaction",font="Helvetica 12 bold", padx=16, pady=4,command=add_entry)
...
exit_button=Button(frame, text="Quit",font='verdana 16 bold', command=quit_app)


header.grid(row=0,columnspan=2)
heading.grid(row=1, column=0,pady=20,columnspan=2)  
button_add.grid(row=2, column=0,padx=50, pady=10) 
button_delete.grid(row=3, column=0,padx=50, pady=10)
button_search.grid(row=4, column=0,padx=50, pady=10)
button_view.grid(row=5, column=0,padx=50, pady=10)
button_update.grid(row=6, column=0,padx=50, pady=10)
add_button.grid(row=2, column=1,padx=50, pady=10)
view_button.grid(row=3, column=1,padx=60, pady=10)
attach_button.grid(row=4, column=1,padx=30, pady=10)
retrieve_button.grid(row=5, column=1,padx=30, pady=10)
